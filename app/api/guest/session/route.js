...
export async function GET(request) {
  try {
    ...
    const { data: guest, error } = await admin.from('guest_sessions').select('jti,invite_id,username,expires_at,revoked_at,current_room_name').eq('jti', payload.jti).maybeSingle();
    ...
    return Response.json({
      ...
      role: 'convidado',
      currentRoom: guest.current_room_name || null,
