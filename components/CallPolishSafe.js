  ...
  useEffect(() => {
    ...
    async function startPreview() {
      if (!navigator.mediaDevices?.getUserMedia) {
        onError?.('Seu navegador não permite acessar câmera e microfone.');
        return;
      }
      try {
        ...
        setMicLevel(0);
        onError?.(error?.name === 'NotAllowedError' ? 'Permissão para câmera e microfone negada.' : error?.message || 'Não foi possível acessar câmera e microfone.');
      }
    }
    startPreview();
    return () => {
...
export function CallExperience({ token, serverUrl, channel, user, rightTab, rightPanelOpen = true, onRightTab, messages, messageText, setMessageText, onSendMessage, onToast, onDisconnect, onRoomMoved, onModerate, onCreateInvite, participantFilter = '', onCallReconnecting, onCallReconnected }) {
  ...
  return <div className={styles.root}>
    <LiveKitRoom token={token} serverUrl={serverUrl} connect audio={joinAudio} video={joinVideo} options={roomOptions} onConnected={handleConnected} onDisconnected={handleDisconnected} onError={(error) => onToast?.({ type: 'error', title: 'Problema na chamada', message: error?.message || 'A conexão foi interrompida. Tentaremos reconectar automaticamente.' })} onMediaDeviceFailure={(failure) => onToast?.({ type: 'error', title: 'Dispositivo indisponível', message: failure?.message || 'Verifique se sua câmera e seu microfone estão disponíveis.' })}>
      <RoomAudioRenderer />
...
function CallLobby({ channel, user, audio, video, setAudio, setVideo, audioDevice, setAudioDevice, cameraDevice, setCameraDevice, onCancel, onJoin }) {
  ...
  return <div className={styles.lobbyShell}><div className={styles.lobbyGlow} /><div className={styles.lobbyCard}>
    ...
    <div className={styles.kicker}>Antes de entrar</div>
    <h2>Entrar em <span>#{channel.name}</span></h2>
    <p>Confira sua câmera e seu microfone antes de entrar. Você poderá alterá-los durante a chamada.</p>
    ...
    <div className={styles.deviceGrid}>
      <button type="button" className={`${styles.deviceChoice} ${audio ? styles.active : ''}`} onClick={() => setAudio(!audio)}><span className={styles.choiceIcon}><Icon name="mic" /></span><span className={styles.choiceCopy}><b>{audio ? 'Microfone ativado' : 'Microfone desativado'}</b><small>{devices.audio.length || 'Nenhum'} dispositivo{devices.audio.length === 1 ? '' : 's'}</small></span><span className={styles.choiceState}>{audio ? 'ON' : 'OFF'}</span></button>
      <button type="button" className={`${styles.deviceChoice} ${video ? styles.active : ''}`} onClick={() => setVideo(!video)}><span className={styles.choiceIcon}><Icon name="camera" /></span><span className={styles.choiceCopy}><b>{video ? 'Câmera ativada' : 'Câmera desativada'}</b><small>{devices.video.length || 'Nenhum'} dispositivo{devices.video.length === 1 ? '' : 's'}</small></span><span className={styles.choiceState}>{video ? 'ON' : 'OFF'}</span></button>
    </div>
    ...
    {audio && <div className={styles.micMeter}><div><span>Teste do microfone</span><b>{micLevel > 8 ? 'Detectando áudio' : 'Fale para testar o microfone'}</b></div><div className={styles.meterTrack}>{Array.from({ length: 16 }, (_, index) => <i key={index} className={index < Math.ceil(micLevel / 7) ? styles.meterActive : ''} />)}</div></div>}
...
function ConnectedCall({ channel, user, rightTab, rightPanelOpen, onRightTab, onCloseRight, messages, messageText, setMessageText, onSendMessage, onToast, onRoomMoved, onModerate, onCreateInvite, participantFilter, onManualDisconnect, onCallReconnecting, onCallReconnected }) {
    ...
    if (mounted.current) {
      participants.forEach((participant) => {
        if (!previous.current.has(participant.identity) && participant.identity !== localParticipant?.identity) {
          onToast?.({ type: 'success', title: 'Alguém entrou na chamada', message: `${displayName(participant)} entrou em #${displayedRoomName}.` });
      ...
      previous.current.forEach((identity) => {
        if (!current.has(identity) && identity !== localParticipant?.identity) {
          onToast?.({ type: 'info', title: 'Participante saiu', message: `${identity.replace(/^guest:/, '')} saiu da chamada #${displayedRoomName}.` });
  ...
  }, [participants, displayedRoomName, localParticipant?.identity, onToast]);
  useEffect(() => {
    ...
    current.forEach((identity) => {
      if (!previousScreenSharesRef.current.has(identity)) {
        ...
        auditCallEvent('screen_share_started', displayName(participant), 'Você começou a compartilhar a tela.');
      }
    });
    previousScreenSharesRef.current.forEach((identity) => {
      if (!current.has(identity)) auditCallEvent('screen_share_stopped', identity.replace(/^guest:/, ''), 'O compartilhamento de tela foi encerrado.');
    });
  ...
  useEffect(() => {
    ...
    if (!reconnectAuditRef.current) {
      ...
      auditCallEvent('call_reconnecting', localParticipant?.identity, 'Conexão perdida temporariamente. Tentando reconectar automaticamente…');
      onCallReconnecting?.();
    }
  ...
  useEffect(() => {
    const handleReconnected = () => {
      ...
      auditCallEvent('call_reconnected', localParticipant?.identity, 'A conexão foi restabelecida.');
      onToast?.({ type: 'success', title: 'Conexão restabelecida', message: 'A chamada foi reconectada.' });
  ...
  useEffect(() => {
    const handleMoved = (roomName, token) => {
      if (!roomName || roomName === displayedRoomName) return;
  ...
  }, [room, displayedRoomName, onRoomMoved]);
  ...
  function showScreenControls() {
    ...
    if (screenControlsTimerRef.current) window.clearTimeout(screenControlsTimerRef.current);
    if (screenFocusSid) screenControlsTimerRef.current = window.setTimeout(() => setScreenControlsVisible(false), 2200);
  }
  function focusScreen(trackRef) {
    setScreenFocusSid(trackRef.publication?.trackSid || null);
    setScreenControlsVisible(true);
    if (screenControlsTimerRef.current) window.clearTimeout(screenControlsTimerRef.current);
    screenControlsTimerRef.current = window.setTimeout(() => setScreenControlsVisible(false), 2200);
  }
  function leaveScreenFocus() {
    setScreenFocusSid(null);
    setScreenControlsVisible(true);
    if (screenControlsTimerRef.current) window.clearTimeout(screenControlsTimerRef.current);
    showToolbar();
  }
  async function toggleScreenFullscreen() {
    try {
      ...
      await screenFocusRef.current?.requestFullscreen?.();
      setScreenControlsVisible(false);
    } catch (error) {
      onToast?.({ type: 'error', title: 'Tela cheia', message: error?.message || 'O navegador não permitiu abrir a tela cheia.' });
    }
  }
  ...
  const localMedia = `${localParticipant?.isMicrophoneEnabled ? 'Microfone ativo' : 'Microfone desativado'} · ${localParticipant?.isCameraEnabled ? 'Câmera ativa' : 'Câmera desativada'}`;
  ...
  return <div className={styles.connected}>
    <section className={styles.callArea}>
      ...
      {!screenFocusSid && <div
        className={styles.toolbarWrap + ' ' + (toolbarVisible ? '' : styles.toolbarHidden)}
      ...
      >
        <CallControls
          ...
          onShowParticipants={() => { onRightTab('participants', true); showToolbar(); }}
          onShowChat={() => { onRightTab('chat', true); showToolbar(); }}
          participantCount={participants.length}
          unreadMessages={unreadMessages}
          moreOpen={moreOpen}
          setMoreOpen={(value) => { setMoreOpen(value); showToolbar(); }}
          reactionsOpen={reactionsOpen}
          setReactionsOpen={(value) => { setReactionsOpen(value); showToolbar(); }}
          onReaction={(emoji) => { sendReaction(emoji); showToolbar(); }}
          onLeave={onManualDisconnect}
        />
      </div>}
    </section>
    <aside className={`${styles.panel} ${rightPanelOpen ? styles.panelOpen : ''} ${screenFocusSid ? styles.panelHiddenForScreen : ''}`}><div className={styles.panelTabs}><button className={rightTab === 'participants' ? styles.activeTab : ''} onClick={() => onRightTab('participants', true)}>Participantes <span>{participants.length}</span></button><button className={rightTab === 'chat' ? styles.activeTab : ''} onClick={() => onRightTab('chat', true)}>Chat{unreadMessages > 0 ? <span className={styles.unreadTab}>{unreadMessages > 99 ? '99+' : unreadMessages}</span> : messages.length > 0 && <i />}</button><button type="button" className={`${styles.panelCloseButton} icon-btn`} onClick={onCloseRight} aria-label="Recolher painel" title="Recolher painel"><Icon name="close" size={16} /></button></div>
      {rightTab === 'participants' ? <div className={styles.panelScroll}><div className={styles.panelTitle}>PARTICIPANTES <span className={styles.panelTitleCount}>{filtered.length}</span></div>{filtered.length ? filtered.map((participant) => { const name = displayName(participant); const local = participant.identity === localParticipant?.identity; const role = participantRole(participant); return <div key={participant.identity} className={`${styles.participantRow} ${participant.isSpeaking ? styles.speakingRow : ''}`}><Avatar name={name} size="sm" status="online" /><div className={styles.participantInfo}><strong>{name}{local ? ' (você)' : ''}</strong><span><i />{roleLabel(role)}{participant.isSpeaking ? ' • falando agora' : participant.isScreenShareEnabled ? ' • compartilhando a tela' : ''}</span></div><div className={styles.participantTools}><span title="Microfone" className={participant.isMicrophoneEnabled ? styles.mediaOn : styles.mediaOff}><Icon name={participant.isMicrophoneEnabled ? 'mic' : 'micOff'} size={12} /></span><span title="Câmera" className={participant.isCameraEnabled ? styles.mediaOn : styles.mediaOff}><Icon name={participant.isCameraEnabled ? 'camera' : 'cameraOff'} size={12} /></span>{participant.isScreenShareEnabled && <span title="Compartilhando a tela" className={styles.mediaOn}><Icon name="monitor" size={12} /></span>}{!local && ['admin', 'membro'].includes(user.role) && <><button className="icon-btn" title={participant.isMicrophoneEnabled ? 'Silenciar microfone' : 'Microfone indisponível'} disabled={!participant.isMicrophoneEnabled} onClick={() => onModerate?.(displayedRoomName, participant.identity, 'mute')}><Icon name="mic" size={13} /></button><button className="icon-btn" title="Desconectar" onClick={() => onModerate?.(displayedRoomName, participant.identity, 'disconnect')}><Icon name="logout" size={13} /></button></>}</div></div>; }) : <EmptyState icon="users" title="Nenhum participante encontrado" description="Tente buscar por outro nome." />}</div> : <div className={styles.chatWrap}><div className={styles.chatSearch}><div className="search-box" style={{ width: '100%' }}><Icon name="search" size={15} /><input className="input" value={chatSearch} onChange={(event) => setChatSearch(event.target.value)} placeholder="Pesquisar mensagens..." /></div></div><div className={styles.chatList} ref={chatListRef}>{filteredMessages.length ? filteredMessages.map((message) => <div className={styles.chatMessage} key={message.id}><Avatar name={message.sender_name} size="sm" /><div className={styles.chatBody}><div><strong>{message.sender_name}</strong><span>{new Date(message.created_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</span></div><p>{message.content}</p></div></div>) : <EmptyState icon="chat" title={chatSearch ? 'Nenhuma mensagem encontrada' : 'Ainda não há mensagens'} description={chatSearch ? 'Tente outro termo de busca.' : 'Envie a primeira mensagem para começar a conversa.'} />}</div>
...
function CallControls({ focusMode, setFocusMode, onShowParticipants, onShowChat, participantCount, unreadMessages = 0, moreOpen, setMoreOpen, reactionsOpen, setReactionsOpen, onReaction, onLeave }) {
  const { localParticipant } = useLocalParticipant();
  const room = useRoomContext();
  useEffect(() => {
    const handler = (event) => {
  ...
  return <div className={styles.toolbar}>
    {button(!!localParticipant?.isMicrophoneEnabled, localParticipant?.isMicrophoneEnabled ? 'Microfone ativado' : 'Ativar microfone', 'mic', () => localParticipant?.setMicrophoneEnabled(!localParticipant.isMicrophoneEnabled))}
    {button(!!localParticipant?.isCameraEnabled, localParticipant?.isCameraEnabled ? 'Câmera ativada' : 'Ativar câmera', 'camera', () => localParticipant?.setCameraEnabled(!localParticipant.isCameraEnabled))}
    {button(!!localParticipant?.isScreenShareEnabled, localParticipant?.isScreenShareEnabled ? 'Parar de compartilhar' : 'Compartilhar tela', 'monitor', () => localParticipant?.setScreenShareEnabled(!localParticipant.isScreenShareEnabled))}
    <button type="button" className={styles.control} onClick={onShowParticipants} title="Ver participantes"><Icon name="users" size={18} /><span>Participantes</span><b className={styles.toolbarCount}>{participantCount}</b></button>
    <button type="button" className={`${styles.control} ${unreadMessages > 0 ? styles.chatAttention : ''}`} onClick={onShowChat} title="Abrir chat"><Icon name="chat" size={18} /><span>Chat</span>{unreadMessages > 0 && <b className={styles.toolbarCount}>{unreadMessages > 99 ? '99+' : unreadMessages}</b>}</button>
    <div className={styles.menuWrap}><button type="button" className={`${styles.control} ${moreOpen ? styles.controlSelected : ''}`} onClick={() => setMoreOpen((value) => !value)} title="Mais opções"><Icon name="settings" size={18} /><span>Mais</span></button>{moreOpen && <div className={styles.moreMenu}><div className={styles.menuTitle}>Dispositivos</div><div className={styles.deviceItem}><span>Selecione a câmera e o microfone nas configurações do navegador.</span></div></div>}</div>
    <div className={styles.menuWrap}><button type="button" className={`${styles.control} ${reactionsOpen ? styles.controlSelected : ''}`} onClick={() => setReactionsOpen((value) => !value)} title="Reações">☺<span>Reações</span></button>{reactionsOpen && <div className={styles.reactionMenu}>{REACTIONS.map((emoji) => <button key={emoji} type="button" onClick={() => { onReaction(emoji); setReactionsOpen(false); }}>{emoji}</button>)}</div>}</div>
    <button type="button" className={`${styles.control} ${focusMode ? styles.controlSelected : ''}`} onClick={() => setFocusMode((value) => !value)} title="Modo de foco"><Icon name="maximize" size={18} /><span>Foco</span></button>
