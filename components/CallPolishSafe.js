...
function ConnectedCall({ channel, user, rightTab, rightPanelOpen, onRightTab, onCloseRight, messages, messageText, setMessageText, onSendMessage, onToast, onModerate, participantFilter }) {
  ...
  useEffect(() => {
    ...
    if (mounted.current) {
      participants.forEach((participant) => { if (!previous.current.has(participant.identity) && participant.identity !== localParticipant?.identity) onToast?.({ type: 'success', title: 'Alguém entrou na call', message: `${displayName(participant)} entrou em #${channel.name}.` }); });
      previous.current.forEach((identity) => { if (!current.has(identity) && identity !== localParticipant?.identity) onToast?.({ type: 'info', title: 'Participante saiu', message: `${identity.replace(/^guest:/, '')} saiu de #${channel.name}.` }); });
    }
  ...
  useEffect(() => { if (!reconnecting) return; const timer = window.setTimeout(() => onToast?.({ type: 'info', title: 'Conexão', message: 'Tentando restaurar a chamada...' }), 1200); return () => window.clearTimeout(timer); }, [reconnecting, onToast]);
  useEffect(() => {
...
function CallControls({ focusMode, setFocusMode, moreOpen, setMoreOpen, reactionsOpen, setReactionsOpen, onReaction }) {
  ...
  const button = (enabled, label, icon, action) => <button type="button" className={`${styles.control} ${enabled ? styles.controlEnabled : styles.controlOff}`} onClick={action} title={label}>{enabled ? <Icon name={icon} size={19} /> : <Icon name="close" size={17} />}<span>{label}</span></button>;
  return <div className={styles.toolbar}>
    {button(!!localParticipant?.isMicrophoneEnabled, localParticipant?.isMicrophoneEnabled ? 'Microfone' : 'Ativar mic', 'mic', () => localParticipant?.setMicrophoneEnabled(!localParticipant.isMicrophoneEnabled))}
    {button(!!localParticipant?.isCameraEnabled, localParticipant?.isCameraEnabled ? 'Câmera' : 'Ativar câmera', 'camera', () => localParticipant?.setCameraEnabled(!localParticipant.isCameraEnabled))}
    {button(!!localParticipant?.isScreenShareEnabled, localParticipant?.isScreenShareEnabled ? 'Parar tela' : 'Compartilhar', 'monitor', () => localParticipant?.setScreenShareEnabled(!localParticipant.isScreenShareEnabled))}
    <div className={styles.menuWrap}><button type="button" className={`${styles.control} ${moreOpen ? styles.controlSelected : ''}`} onClick={() => setMoreOpen((value) => !value)} title="Mais opções"><Icon name="settings" size={18} /><span>Mais</span></button>{moreOpen && <div className={styles.moreMenu}><div className={styles.menuTitle}>Dispositivos</div><div className={styles.deviceItem}><span>Troque câmera ou microfone no navegador.</span></div></div>}</div>
