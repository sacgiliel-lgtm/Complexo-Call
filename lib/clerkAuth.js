...
export async function syncClerkProfile() {
  ...
  // O username salvo no perfil é o nome exibido da aplicação.
  // O nome do Clerk serve como valor inicial durante a criação do perfil,
  // mas não deve sobrescrever alterações feitas pelo próprio usuário.
  if (!profile.username) {
    const desiredUsername = user.username || fallbackUsername(user, email);
    if (desiredUsername) {
