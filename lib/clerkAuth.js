...
export function getEmailLoginExternalId(email) {
  const normalizedEmail = String(email || '').trim().toLowerCase();
  const secret = process.env.CLERK_SECRET_KEY?.trim();
  if (!normalizedEmail || !secret) return '';
  return `cpx-email-${crypto.createHmac('sha256', secret).update(normalizedEmail).digest('hex')}`;
}
...
export async function syncClerkProfile() {
  ...
  const identityEmailHash = email ? crypto.createHash('sha256').update(email).digest('hex').slice(0, 12) : '';

  console.info('Clerk profile sync identity:', {
    hasIdentityEmail: !!identity.email,
    hasMetadataEmail: !!metadataEmail,
    identityEmailHash,
    clerkEmailCount: user?.emailAddresses?.length || 0,
  ...
  if (repairEmail) {
    ...
    const loginExternalId = getEmailLoginExternalId(pendingEmail);
    console.info('Clerk profile sync email identifier:', {
      emailHash: crypto.createHash('sha256').update(pendingEmail).digest('hex').slice(0, 12),
      externalIdMatches: currentUser.externalId === loginExternalId,
      hasExternalId: !!currentUser.externalId,
    });
    if (loginExternalId && currentUser.externalId !== loginExternalId) {
      try {
        await client.users.updateUser(userId, { externalId: loginExternalId });
      } catch (error) {
        console.warn('Clerk profile sync: could not set email login identifier.', {
          userId,
