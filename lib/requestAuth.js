...
export async function getRequestActor(request) {
  ...
  if (authorization?.startsWith('Bearer ')) {
    ...
    const { data: profile, error: profileError } = await admin
      .from('profiles')
      .select('id,username,role,status,presence_status,last_seen_at')
      .eq('id', user.id)
      .single();
  ...
  return { ok: true, type: 'guest', id: `guest:${guest.jti}`, username: guest.username, role: 'convidado', guest, admin };
}
